...
const connectAccount = async (req, res) => {
  ...
  if (!email || !imapPassword || !imapHost) {
    res
      .status(400)
      .json({ message: "Please provide the email, password, and the host" });
    return;
  }
...
const getEmails = async (req, res) => {
  try {
    ...
    console.log(
      "Decrypted password (first 3 chars):",
      originalPassword.slice(0, 3),
      "***"
    );
...
const getAccounts = async (req, res) => {
  try {
    const accounts = await ConnectedAccount.find({ user: req.user.id }).select(
      "email _id provider"
    );
    res.json(accounts);
  } catch (error) {
...
// @desc    Disconnect/Remove an account
// @route   DELETE /api/accounts/:id
// @access  Private
const deleteAccount = async (req, res) => {
  try {
    // We strictly match both the Account ID (_id) AND the User ID (user)
    // This prevents User A from deleting User B's account
